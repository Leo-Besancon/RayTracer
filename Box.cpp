...
Box::Box(Material material, Vector min, Vector max, const std::vector<Vector*> & v, const std::vector<std::pair<double, double>*> & vt, const std::vector<Vector*> & vn, std::vector<Face*> f, int recursion_level, unsigned char* texture, int  texture_width, int texture_height) :
	_min(min), _max(max), _recursion_level(recursion_level), _v(v), _vt(vt), _vn(vn), _texture(texture), _texture_width(texture_width), _texture_height(texture_height)
{
	_material = material;
		...
		for (int i = 0; i < n_box; i++)
		{
			for (int j = 0; j < n_box; j++)
			{
				for (int k = 0; k < n_box; k++)
				{
					...
					Box* box = new Box(_material, min, max, _v, _vt, _vn, _f, _recursion_level + 1, _texture, _texture_width, _texture_height);
