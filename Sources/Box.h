...
public:
	Box(Material material, Vector min, Vector max, const std::vector<Vector*>& v, const std::vector<std::pair<double, double>*>& vt, const std::vector<Vector*>& vn, std::vector<Face*> f, int recursion_level, unsigned char * texture = nullptr, int texture_width = 0, int texture_height = 0, bool texture_procedural = false);
	~Box();
	...
	Vector get_texture_procedural_color(Vector n) const;
