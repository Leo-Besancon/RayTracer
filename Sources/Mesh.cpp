...
void Mesh::constr(Material material, std::string filename)
{
...
{
	constr(material, filename);

	if(texture_filename == "PROCEDURAL")
	{
		std::cout << " Using procedural texture" << std::endl;
		_texture_procedural = true;
	}
	else
	{
		std::cout << " Using Texture file " << texture_filename.c_str() << std::endl;
