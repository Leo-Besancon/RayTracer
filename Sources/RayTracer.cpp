...
void RayTracer::createScene()
{
	...
	//Mesh* m = new Mesh(Material(), "T-Rex Model", "PROCEDURAL");
	//Mesh* m = new Mesh(Material::createMirror(Vector(1,1,1)), "T-Rex Model", "Texture.bmp");
	//Mesh* m = new Mesh(Material::createDiffuse(Vector(1.,1.,0.2)), "T-Rex Model");
#ifdef MAIN_LOGS
...
TODO :
- Camera _camera -> std::vector<Camera> _cameras
- Construct the cameras (just translated perpendicularly to the wanted direction, not rotated)
- Get the Field view of all the cameras, and get the intersection
- Create a grid of this intersection
- For each column j, take the _cameras[j % Nb_cameras] to compute the ray
That's it !
...
void RayTracer::render_one_leticular_frame(int frame)
{
